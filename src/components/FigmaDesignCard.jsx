...
export default function FigmaDesignCard({ design, index }) {
  return (
    <motion.div
    ...
    >
      ...
      <div className="relative aspect-[16/10] overflow-hidden bg-black/40">
        ...
        <div className="absolute top-4 left-4">
           <div className="flex items-center gap-2 px-3 py-1.5 bg-black/60 border border-white/10 backdrop-blur-xl rounded-full">
              <FaFigma className="text-[#d4a843] text-[10px]" />
              <span className="text-[8px] font-mono text-white/60 uppercase tracking-widest">Design_0{design.id}</span>
           </div>
        </div>
      </div>
      ...
      <div className="p-8 space-y-6">
        <div className="space-y-2">
           <div className="flex items-center gap-2">
              <span className="text-[9px] font-mono text-[#d4a843] uppercase tracking-[0.3em] font-black">{design.category}</span>
              <div className="w-1 h-1 rounded-full bg-white/20" />
              <span className="text-[9px] font-mono text-white/30 uppercase tracking-widest">FIGMA_SOURCE</span>
           </div>
      ...
      {/* Aesthetic Accents */}
      <div className="absolute bottom-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-[#d4a843]/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
    </motion.div>
  );
}
